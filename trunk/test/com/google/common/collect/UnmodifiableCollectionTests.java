...
public class UnmodifiableCollectionTests {
  ...
  public static <E> void assertMultisetIsUnmodifiable(Multiset<E> multiset,
    ...
    Multiset<E> copy = new LinkedHashMultiset<E>(multiset);
    ...
    try {
      multiset.removeAllOccurrences(sampleElement);
      fail("removeAllOccurrences succeeded on unmodifiable collection");
    } catch (UnsupportedOperationException expected) {
    }
    assertCollectionsAreEquivalent(multiset, copy);
  ...
  public static <K, V> void assertMultimapIsUnmodifiable(
    ...
    Multimap<K, V> multimap2 = Multimaps.newArrayListMultimap();
